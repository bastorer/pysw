...
import Fortran_Code_vanish as Fortran_Code

# Compute the flux terms
def fv_sw_flux_vanish(sim):
    ...
    for ii in range(sim.Nz):
        ...
        # x fluxes
        if sim.Nx > 1:
            Fm,Fp = Fortran_Code.flux_split_x(h,uh,vh,sim.g,sim.np,sim.min_depth) 
