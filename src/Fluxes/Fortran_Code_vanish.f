C     File: Fortran_Code.f
      ...
      SUBROUTINE FLUX_SPLIT_X(FM,FP,H,UH,VH,G,N,MIND,NX,NY)
...
C         The eigenvalues
          ...
          LAM2_ALL(I,J) = U/(D+EPS) + SQRT(G*D+G*MINDN/(D**(N-1)))
          LAM3_ALL(I,J) = U/(D+EPS) - SQRT(G*D+G*MINDN/(D**(N-1)))
...
C         Compute the alpha matrix
          ...
          ALPHA(Iu,Iv) = 0.0
          TMP = V/((LAM2-LAM3)*D)
          ALPHA(Iv,Ih) = -TMP*(LB1*(lam2 - lam3) + LB2*lam3 - LB3*lam2)
          ALPHA(Iv,Iu) =  TMP*(LB2-LB3)
          ALPHA(Iv,Iv) = LB1
